...
这里既包含 tokenizer 相关工具，也包含 chunk 合并、表格处理、位置标注等逻辑。
读建库链路时，最值得关注的是 `tokenize_chunks`、`tokenize_table`、`naive_merge`
和 `naive_merge_docx` 这些函数。
"""
...
def split_with_pattern(d, pattern: str, content: str, eng) -> list:
    ...
    try:
        ...
        # 回退策略：如果正则非法，则把整段内容当作一个 chunk 返回。
        dd = copy.deepcopy(d)
...
def attach_media_context(chunks, table_context_size=0, image_context_size=0):
    """
    给表格/图片类 chunk 挂接周围文本上下文。

    如果 chunk 带位置信息，则优先按位置排序；否则保持原始顺序。
    """
    from . import rag_tokenizer
...
def append_context2table_image4pdf(sections: list, tabls: list, table_context_size=0, return_context=False):
    ...
    for i, item in enumerate(sections):
        if isinstance(item, (tuple, list)):
            ...
            txt = item
            poss = ""
        # 普通模式：`(text, "@@...##")`，位置直接塞在字符串标签里。
        # 手工模式：`(text, sec_id, poss_list)`，位置是 `(page, left, right, top, bottom)` 列表。
        # 论文模式：`(text_with_@@tag, layoutno)`，如果文本里带 `@@` 标记，则从文本中反解析坐标。
...
def tree_merge(bull, sections, depth):
    ...
    # 过滤掉 PDF section 中的位置信息标签，只保留文本内容参与后续切分。
...
def _build_cks(sections, delimiter):
    ...
    # 提取用户用反引号声明的自定义分隔符，例如 `##`、`---` 等。
    custom_delimiters = [m.group(1) for m in re.finditer(r"`([^`]+)`", delimiter)]
    has_custom = bool(custom_delimiters)

    if has_custom:
        # 先转义特殊字符，再按“长分隔符优先”拼成 alternation 正则。
        custom_pattern = "|".join(
    ...
    for text, image, table in sections:
        # 统一把文本转成字符串，并补一个换行，方便和前后上下文自然拼接。
        if not text:
            text = ""
        else:
            text = "\n" + str(text)
        ...
        if image:
            # 图片块：文本只保留上下文提示，图片本身单独挂在 chunk 上。
            idx = len(cks)
            cks.append({
                "text": text,
                "image": image,
...
def naive_merge_docx(
    ...
    """针对 Docx/PDF 混合文本、表格、图片的场景做 chunk 合并。"""
    if not sections:
        return [], []
